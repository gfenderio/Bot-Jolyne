...
// Exported so the mock-send script renders the EXACT same embed as the real cron.
export function buildDailyEmbed(events: PacePackEvent[], start: Date): EmbedBuilder | null {
...
export function startPacePackScheduler(client: Client) {
  ...
  cron.schedule("5 0 * * *", async () => {
    try {
      ...
      const embed = buildDailyEmbed(events, start);
      if (!embed) return;
